...
const baseIpsSMZ3 = require('resources/sm.ips.gz')
const baseIpsSM = require('resources/sm.ips.gz')
...
const Z3Sprite = ({ index }) => {
  return styled.option`
    ...
    background-position: -${index * 16}px 0;
    background-image: url(${process.env.PUBLIC_URL}/sprites/z3.png);
  `
}

const SMSprite = (index) => styled(Z3Sprite({ index }))`
  background-image: url(${process.env.PUBLIC_URL}/sprites/sm.png);
`

const JumpSprite = (which) => styled.span`
  width: 17px;
  height: 17px;
  background-size: auto 17px;
  background-image: url(${process.env.PUBLIC_URL}/sprites/jump_${which}.png);
`
...
export default function Patch(props) {
  ...
  useEffect(() => {
    attempt(async () => {
      ...
      if ((!(game.id === 'z3') || fileDataLTTP !== null) && fileDataSM !== null) {
  ...
  function constructFileName() {
    ...
    function smParts({ logic, placement }) {
      return [`L${logic[0]}`, `I${placement[0]}`]
    }
  ...
  const component =
    patchState === 'upload' ? (
      ...
      <Form onSubmit={(e) => e.preventDefault()}>
        <Row className="mb-3">
          <Col md={game.id === 'smz3' ? 10 : 6}>
            <SpriteSettings
              game={game}
              sprites={sprites}
              settings={{ z3Sprite, smSprite, smSpinjumps }}
            ...
            <Col md="5">
              <InputGroup prefix="Heart Beep">
                <Input
                  type="select"
                  value={z3HeartBeep}
                  onChange={(e) => onZ3HeartBeepChange(e.target.value)}
                >
            ...
            <Col md="4">
              <InputGroup prefix="Heart Color">
                <Input
                  type="select"
                  value={z3HeartColor}
                  onChange={(e) => onZ3HeartColorChange(e.target.value)}
                >
        ...
        <Row>
          <Col className="d-flex align-items-center" md="6">
            <Button color="primary" onClick={onDownloadRom}>
              Download ROM
            </Button>
  ...
  return (
    <InputGroup className="flex-nowrap" prefix="Play as">
      {game.id === 'z3' && (
        ...
        >
          {sprites.z3.map(({ title }, i) => (
            <SpriteOption key={title}>
              {Z3Sprite({ index: i })}
              {title}
            </SpriteOption>
          ))}
      ...
      <InputGroupAddon addonType="append">
        <InputGroupText tag={Label} title="Enable separate space/screw jump animations">
          <Input
            ...
            addon={true}
            checked={smSpinjumps}
            onChange={props.onSpinjumpToggle}
          />{' '}
