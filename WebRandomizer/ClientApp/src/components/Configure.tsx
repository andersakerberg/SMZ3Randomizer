...
export default function Configure(props) {
  ...
  const [randomizer, setRandomizer] = useState({} as any)
  ...
  const content =
    randomizer && !gameMismatch ? (
      ...
      <MessageCard error={true} title="Something went wrong :(" msg={errorMessage} />
    ) : gameMismatch ? (
      <MessageCard
        error={true}
        title="This is not quite right :O"
        msg={
          <a href={adjustHostname(document.location.href, gameId)}>Go to the correct domain here</a>
        }
      />
    ) : (
      <MessageCard error="" title="Create new randomized game" msg="Please wait, loading..." />
    )
    ...
    return map(range(parseInt(options.players)), (p) => (
      <InputGroup prefix={`Name ${p + 1}`}>
        <Input
          ...
          pattern=".*[A-Za-z\d].*"
          onChange={(e) => {
            playerPatternValidity(e.target)
