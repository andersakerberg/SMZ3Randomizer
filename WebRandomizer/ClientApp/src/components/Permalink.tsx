...
export default function Permalink(props) {
  ...
  const content =
    seed && !gameMismatch ? (
      <>
        <Card className="mb-3">
          <CardHeader className="bg-primary text-white">{seed.gameName}</CardHeader>
          <CardBody>
            <Row>
      ...
      <MessageCard
        ...
        msg={
          <a href={adjustHostname(document.location.href, seed.gameId)}>
            Go to the correct domain here
          </a>
        }
      />
    ) : (
      <MessageCard error="" title="Game information" msg="Please wait, loading..." />
    )
