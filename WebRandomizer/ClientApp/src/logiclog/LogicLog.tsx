...
const StyledCardHeader = styled(CardHeader)`
  background-color: rgba(0, 0, 0, ${({ level }) => 0.03 * level});
`
...
const ToggleButton = styled(Button)`
  &.btn-light {
    ...
    &.focus {
      ...
      box-shadow: 0 0 0 0.2rem /* $btn-focus-width */ rgba(180, 186, 191, 0.5); /* mix(color, border, 15%), .5 */
    }
  }
`
...
export default function LogicLog() {
  ...
  const { normal, hard } = last(parts) as any
  ...
  const logicButton = (logic: string, name: string) => (
    <InputGroupAddon addonType="append">
      <ToggleButton color={logic === name ? 'primary' : 'light'} onClick={() => setSMLogic(name)}>
        {capitalize(name)}
      </ToggleButton>
    </InputGroupAddon>
  )
  ...
  const log = (
    <Card>
      {bars
        .map((bar, i) => [bar, bars.slice(1, i + 1).map((x: any) => x.name)])
        .map(([bar, path], i) => (
          <StyledCardHeader key={i} level={bars.length - i}>
            <Nav card tabs>
              {(bar as any).tabs.map((tab: any) => {
                return (
                  <NavItem key={tab.name}>
                    <NavLink
                      className={classnames({ active: name === tab.active })}
                      onClick={() => setTabState(activate(tabState, [...tab, tab.name]))}
                    >
                      {tab.name}
                    </NavLink>
                  </NavItem>
...
function active(state: any, content: any): any[] {
  state = state || {}
  content = content || {}
  ...
  if (content.tabs) {
    ...
    return [{ ...content, active: tab.name }, ...active(state[tab.name], tab)]
  }
  return [{ ...content }]
}

function activate(state: any, tabs: any[]): any {
