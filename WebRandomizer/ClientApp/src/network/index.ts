...
import {
  create_message, connect, send, clearBusy, readData, writeData,
} from '../snes/usb2snes';
...
export default class Network {
  ...
  MessageBaseAddress: any | null;

  ItemsBaseAddress: any | null;

  GameServiceHost: any | null;

  session: { guid: any | null; state: number; data: any | null };

  clientData: any | null;

  device: { state: number; version: string; list: any; selecting: boolean; selected: any | null };

  hub: { state: number };

  game: { state: number; inEvents: any[]; outEvents: any[]; writeQueue: any[] };

  react: any | null;

  constructor(sessionGuid: string, gameServiceHost: any, react: any) {
    ...
    this.itemInPtr = -1;
    this.itemOutPtr = -1;
    this.eventLoopTimer = new NodeJS.Timeout();
  ...
  async startSession() {
    ...
    try {
      const response = await fetch(
        `https://${this.GameServiceHost}/api/multiworld/session/${this.session.guid}`,
      );
      ...
      if (registered) {
        ...
        if (this.clientData === null) {
          ...
          if (clientGuid !== null && clientGuid !== '') {
            ...
            const client = await this.connection?.invoke(
              'RegisterPlayer',
              this.session.guid,
              clientGuid,
            );
  ...
  async onRegisterPlayer(clientGuid: string) {
    /* If we're connected to USB2SNES, disconnect first */
    if (this.device.state === 1) {
      this.game = {
        state: 0, inEvents: [], outEvents: [], writeQueue: [],
      };
      this.device = {
        state: 0, version: '', list: null, selecting: false, selected: null,
      };
      this.socket.close();
      this.updateState();
    }
  ...
  onConnect = async () => {
    try {
      ...
      const response = ((await send(create_message('DeviceList', []))) as unknown) as any;
      ...
      if (deviceList.Results.length === 1) {
        ...
        this.device = {
          ...this.device, selecting: true, list: deviceList, selected: firstDevice,
        };
        this.updateState();
        ...
        if (attached) {
          this.device = {
            ...this.device, selecting: false, list: null, selected: null,
          };
          this.updateState();
        }
      }
  ...
  async attachDevice(device: any) {
    try {
      const attached = await send(create_message('Attach', [device]), true, 500);
      if (attached === true) {
        const response = ((await send(create_message('Info', []))) as unknown) as any;
  ...
  async detectGame() {
    const mappings = this.session.data.seed.gameId === 'smz3'
      ...
      : [
        /* Super Metroid */ [0x1c4f00, 0xe01e00, 0xe02000] /* SNES / Snes9x */,
    ...
    for (let i = 0; i < mappings.length; i++) {
      ...
      const seedData = (await readData(addr, 0x50)) as any;
  ...
  async syncSentItems() {
    try {
      const snesItemSendPtrs = (await readData(this.ItemsBaseAddress + 0x680, 0x04)) as any[];

      this.itemInPtr = ushort_le_value(snesItemSendPtrs, 0x00) ?? 0;
      const snesItemOutPtr = ushort_le_value(snesItemSendPtrs, 0x02);

      while (this.itemInPtr < snesItemOutPtr) {
        const itemAddress = this.itemInPtr ?? 0 * 0x08;
        const message = (await readData(this.ItemsBaseAddress + 0x700 + itemAddress, 0x08)) as any[];
        try {
          const ok = await this.handleItemMessage(message);
          if (ok) {
            this.itemInPtr += 1;
            await writeData(
              this.ItemsBaseAddress + 0x680,
              new Uint8Array(ushort_le_bytes(this.itemInPtr)),
            );
          } else {
  ...
  async handleItemMessage(msg: any[]) {
    ...
    return this.sendItem(worldId, itemId, itemIndex, seq);
  }

  async sendItem(worldId: string, itemId: string, itemIndex: number, seq: any) {
    try {
      return await this.connection?.invoke(
        'SendItem',
        this.session.data.guid,
        worldId,
        itemId,
        itemIndex,
        seq,
  ...
  async syncReceivedItems() {
    try {
      ...
      const snesItemSendPtrs = (await readData(this.ItemsBaseAddress + 0x600, 0x04)) as any[];
      this.itemOutPtr = ushort_le_value(snesItemSendPtrs, 0x02);

      /* Ask the server for all items from our last known item sequence */
      const events = await this.connection?.invoke(
        'GetEvents',
        this.session.data.guid,
        'ItemReceived',
        this.itemOutPtr,
  ...
  async sendItemMessage(data: any) {
    try {
      await writeData(this.ItemsBaseAddress + this.itemOutPtr * 0x04, new Uint8Array(data));
      this.itemOutPtr += 1;
      await writeData(
        this.ItemsBaseAddress + 0x602,
        new Uint8Array(ushort_le_bytes(this.itemOutPtr)),
      );
      return true;
    } catch (error) {
      return false;
  ...
  async readMessages() {
    ...
    try {
      const snesMsg = (await readData(this.MessageBaseAddress + 0x100, 0x090)) as any;
...
function ushort_le_value(array: any[], offset: number) {
  return array[offset] + (array[offset + 1] << 8);
}

function ushort_le_bytes(x: any) {
